...
const QUALITY_VERSION = 'v10.11-frozen-set-adaptive-v7';
const CALIBRATION_VERSION = 'style-v2';
const BUILD_ENGINE_VERSION = 'v10.11-frozen-set-build-v7';
const QUESTION_ENGINE_VERSION = 'v10.11-frozen-set-v7';
const ADAPTIVE_GENERATION_VERSION = 'adaptive-prefetch-v1';
  ...
  return {
    ...
    verification: {
      ...
      models: {
        ...
        solverA: 'not-used:frozen-set-v7',
        solverB: 'not-used:frozen-set-v7',
        solverC: 'not-used:frozen-set-v7',
      ...
      blindDemand: {
        ...
        observedLevel: q.level,
        shortcut: 'none',
        shortcutReason: 'v10.11 FROZEN v7 자동 품질 게이트 및 필요 시 표적 보정을 통과함',
...
async function startFrozenSetGeneration(assets: Awaited<ReturnType<typeof frozenSetAssets>>, targets: BuildTarget[]) {
  return openAiStartBackground({
    model: GPT_MODELS.generator,
    schemaName: 'frozen_set_questions_v7',
    ...
    async ctx => {
      ...
      try {
        return await withKeyedLock(`passage:${passageId}`, async () => {
          ...
          if (
            b.trial !== true &&
            QUESTION_ENGINE_VERSION === 'v10.11-frozen-set-v7'
          )
      ...
      try {
        ...
        return await withKeyedLock(`passage:${passageId}`, () =>
          withKeyedLock(`progress:${uid}:${passageId}`, async () => {
            ...
            delete (pr as Progress & { id?: string }).id;
            const q = studyQs.find(question => question.id === questionId);
            ...
            let next: (Q & { id: string }) | undefined;
            if (!done) {
